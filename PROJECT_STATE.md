...
- Claude Haiku 6M Chart Triage Stage - added and verified, disabled by default
...
### Claude Haiku 6M Chart Triage Stage

- Added a separate `haiku_chart_triage.py` stage for first-pass visual review of one standardized 6M daily chart per ticker.
- The stage is optional and disabled by default behind `--haiku-chart-triage`; it does not replace deterministic detectors or the existing final Claude analysis path.
- Haiku triage can load candidates from existing detector candidate CSV/JSON files, focus gate audit CSV files, or detector output generated by `--detectors-from-audit`.
- Default source selection is `priority`: detector `chart_needed` rows when detector candidates are available, otherwise passed focus-gate rows, otherwise primary-gated rows.
- Added prompt files under `prompts/`: `STRATEGY_MASTER.md`, `HAIKU_CHART_TRIAGE_PROMPT.md`, and `OUTPUT_SCHEMA.md`.
- Added `strategy_sources/README.md` as the configurable in-repo location for local Blueprint PDFs, transcripts, and example screenshots; local Desktop source paths are not hardcoded.
- Added `strategy_ingestion.py` for one-time PDF/transcript ingestion into a durable `prompts/STRATEGY_MASTER.md`; this extracts local source text and optionally uses Claude once to summarize it.
- Added `pypdf` to `requirements.txt` for local PDF extraction.
- Ran one-time Blueprint PDF ingestion from the provided local PDF path with `strategy_ingestion.py`; extracted 51,554 source characters to `strategy_sources/blueprint_extracted.txt` and generated `prompts/STRATEGY_MASTER.md` using `claude-haiku-4-5`.
- Added `generate_haiku_triage_chart()` in `chart_generator.py` to produce only one clean 6M daily candlestick chart with volume, EMA8/EMA21/EMA50, and optional trigger/stop reference lines.
- Added CLI options for Haiku triage: `--haiku-chart-triage`, `--haiku-triage-limit`, `--haiku-workers`, `--haiku-review-source`, `--haiku-chart-timeframe 6M`, `--haiku-chart-only-6m`, `--skip-haiku-cache`, `--haiku-source-file`, `--haiku-prompts-dir`, `--strategy-source-dir`, and `--haiku-chart-dir`.
- Before Haiku calls, the stage prints chart count, worker count, 5-second/request sequential and parallel runtime estimates, token range, and dollar cost estimate when pricing environment variables are configured.
- Output is saved as CSV, JSON, and Markdown: `reports/haiku_chart_triage_YYYY-MM-DD_HHMM.csv`, `reports/haiku_chart_triage_YYYY-MM-DD_HHMM.json`, and `reports/haiku_chart_triage_report_YYYY-MM-DD_HHMM.md`.
- Anthropic prompt caching is implemented on a best-effort basis with `cache_control` on the static strategy/prompt/schema prefix; `--skip-haiku-cache` disables it.
- Verified `py_compile` for changed and related modules using `PYTHONPYCACHEPREFIX=/private/tmp/trading_system_pycache`.
- Verified a 5-chart detector-source dry run with `venv/bin/python main.py --haiku-chart-triage --haiku-source-file reports/detector_candidates_2026-06-03_2125.csv --haiku-triage-limit 5 --dry-run --output-dir reports`; Claude was not called and charts/reports were generated.
- Verified a 5-chart audit-source dry run with `venv/bin/python main.py --analyze-from-audit reports/focus_gate_audit_2026-06-03_2053.csv --haiku-chart-triage --haiku-triage-limit 5 --dry-run --output-dir reports`; Claude was not called and charts/reports were generated.
- Verified a real 5-chart Haiku test with `venv/bin/python main.py --haiku-chart-triage --haiku-source-file reports/detector_candidates_2026-06-03_2125.csv --haiku-triage-limit 5 --haiku-workers 3 --output-dir reports`; output parsed successfully with 2 KEEP, 3 MAYBE, 0 REJECT, and 0 parse errors.
- After PDF ingestion, verified a one-chart real Haiku call against the PDF-derived `STRATEGY_MASTER.md`; WERN returned valid JSON with `decision=KEEP`, `confidence=HIGH`, `setup_type=big_base_near_highs`, and 0 parse errors.
- Synced user-refined prompt files into `prompts/STRATEGY_MASTER.md`, `prompts/HAIKU_CHART_TRIAGE_PROMPT.md`, and `prompts/OUTPUT_SCHEMA.md`.
- Verified the refined prompt set with a 5-chart dry run and a 5-chart real Haiku run using `reports/detector_candidates_2026-06-03_2125.csv`; the real run returned 4 KEEP, 1 MAYBE, 0 REJECT, and 0 parse errors in 8 seconds.
